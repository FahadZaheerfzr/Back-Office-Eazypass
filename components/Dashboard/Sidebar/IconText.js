...
export default function IconText({ icon, name, unread, clickEvent, active, footer_nav }) {
  ...
  if (footer_nav){
    return (
      <div aria-hidden role={"button"} className={`w-full md:mt-5 lg:mt-6 cursor-pointer flex items-center justify-between md:pl-[10%] lg:pl-[10%] xl:pl-[20%] font-Poppins text-base 
  ...
  return (
    ...
    ${active ? 'text-black border-r-4 border-black' : 'text-[#7C8DB5]'} `}
      ...
      {unread > 0 ?
        <div className='flex justify-center items-center w-5 h-5 mr-[15%] rounded-full bg-[#A6231C]'>
          <span className='text-white text-xs'>{unread}</span>
        </div> : null
      }
    </div>
