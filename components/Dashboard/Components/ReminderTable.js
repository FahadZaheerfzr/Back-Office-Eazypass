...
const warning_colors = {
    "Envoyé" : "#279A44",
    "Envoyer ?" : "#E59523",
    "Ne pas envoyer" : "#A6231C",
}

const days_colors = {
    "3 j non déclarés" : "#347AE2",
    "5 j non déclarés" : "#A6231C",
}
                                ...
                                <tbody className="divide-y divide-gray-200">
                                    {rows.map((row) => (
                                        <tr key={row.id}>
                                            ...
                                            <td className="whitespace-nowrap text-center py-4 px-3 text-xs text-black">{row.semaine}</td>
                                            ...
                                            <td className="  text-center py-4 px-3 text-xs text-black">
                                                <div className='flex items-center justify-center  box-shadow-style '>
                                                    <div className={`w-2 h-2 bg-[${days_colors[row.days]}] rounded-full`} 
                                                        style={{background: days_colors[row.days]}}
                                                    />
                                                    <span className='ml-2'>{row.days}</span>
                                                </div>
                                            </td>
                                            <td className="flex items-center justify-center text-center py-4 px-3 text-xs text-black">
                                                <div className=' box-shadow-style flex items-center'>
                                                    <div className={`w-2 h-2 bg-[${warning_colors[row.warning]}] rounded-full`} />
                                                    <span className='ml-2'>{row.warning}</span>
                                                </div>
                                            </td>
